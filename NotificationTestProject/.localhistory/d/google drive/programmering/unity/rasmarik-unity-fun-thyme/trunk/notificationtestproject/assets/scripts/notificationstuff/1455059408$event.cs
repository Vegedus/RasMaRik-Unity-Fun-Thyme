...
public class Event : Singleton<Event>
{
    ...
    //Used for invoking directly from other unityEvents. Hardcoded strings & reflection is bad though (can break by refactoring), so only for the lazy.
    public void invokeByName(String eventName)
    {
        FieldInfo field = typeof(Event).GetField(eventName);
        ((UnityEvent)field.GetValue(this)).Invoke();
    }
